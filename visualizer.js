...
// Enhanced Node class with more sophisticated visuals
class Node {
    ...
    draw() {
        ...
        // Pulsing ring for high activation
        if (intensity > 0.7) {
            ctx.beginPath();
...
// Enhanced Connection class with dynamic visuals
class Connection {
    ...
    draw() {
        ...
        // Subtle pulse along connection
        const pulse = Math.sin(time * 0.05 + this.pulsePhase) * 0.1 + 0.9;
        
        ctx.beginPath();
        ctx.lineWidth = width * pulse;
        
        // Different colors and effects for different connection types
...
// Enhanced DataPulse class with spectacular visuals
class DataPulse {
    ...
    draw() {
        ...
        // Draw sparkles around main pulse
...
function initNetwork() {
    ...
    for (let layer = 0; layer < layers; layer++) {
        ...
        // Create more organic node positioning
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        // Forward connections with distance-based probability
...
function animate() {
    ...
    // Auto mode with more sophisticated patterns
    if (autoMode) {
        if (Math.random() < 0.04) {
            if (Math.random() < 0.8) {
                triggerRandomPulse();
